...
def resolve_password_file(script_path):
    """根据脚本路径解析密码文件位置：脚本所在目录/../../config/password.json"""
    base = script_path.parent
    password_file = (base / "../../config/password.json").resolve()
...
def get_root_startup_param(password_file):
    ...
    if not password_file.exists():
        ...
        print("请确保文件位于 config/password.json 并包含 'lite-forever' 密码。", file=sys.stderr)
        return None
    try:
